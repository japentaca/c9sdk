...
function Consumer() {
    ...
    function storeStream(stream) {
        ...
        while ("nextStreamID" in streams) { nextStreamID = (nextStreamID + 1) % 10000; }
        var id = nextStreamID;
        streams[id] = stream;
        stream.id = id;
        if (stream.readable) {
    ...
    function makeStreamProxy(token) {
        ...
        if ("readable" in token) stream.readable = token.readable;
        if ("writable" in token) stream.writable = token.writable;
